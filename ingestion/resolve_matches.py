...
from database.db import get_pending_signals, update_signal_result
import logging
...
def _extract_closing_odds(odds_payload: dict | None, fixture: dict, bet_on: str) -> float | None:
    """Pick best available closing odds for bet_on from AllSports Home/Away odds map."""
    if not odds_payload:
        return None
    from utils import normalize_player_name

    first = normalize_player_name(fixture.get("event_first_player", ""))
    second = normalize_player_name(fixture.get("event_second_player", ""))
    bet_norm = normalize_player_name(bet_on or "")

    side = None
    if bet_norm and bet_norm == first:
        side = "Home"
    elif bet_norm and bet_norm == second:
        side = "Away"
    if side is None:
        return None

    prices = odds_payload.get("Home/Away", {}).get(side, {})
    values = []
...
def resolve_pending_signals(
    ...
    if force_signal_id and not forced_outcome:
        ...
        return _summary(ok=False, error="--force requires --outcome (WON, LOST, or VOID)")
    if forced_outcome and not force_signal_id:
        _abort("--outcome requires --force SIGNAL_ID", cli_mode)
        return _summary(ok=False, error="--outcome requires --force SIGNAL_ID")
    ...
    if force_signal_id:
        try:
            ...
            return _summary(ok=False, error="--force SIGNAL_ID must be an integer")

        if forced_outcome not in {"WON", "LOST", "VOID"}:
            _abort("--outcome must be one of WON, LOST, VOID", cli_mode)
            return _summary(ok=False, error="--outcome must be one of WON, LOST, VOID")
        ...
        return _summary(ok=True, resolved=1)
    ...
    try:
        ...
        if data.get("success") == 1:
            ...
            return _summary(
                ok=False,
                pending=len(pending),
                error="fixtures_api_unsuccessful_response",
            )
        ...
        return _summary(
            ok=False,
            pending=len(pending),
            error=f"fixtures_request_failed: {e}",
        )

    odds_map = {}
    try:
        odds_resp = requests.get(ALLSPORTS_BASE, params={
            "met": "Odds",
            "APIkey": ODDS_API_KEY,
            "from": min_date,
            "to": max_date,
        ...
        if odds_data.get("success") == 1:
            raw_odds = odds_data.get("result", {}) or {}
            odds_map = {str(k): v for k, v in raw_odds.items()}
    except Exception as e:
        logger.warning(f"Odds fetch failed during resolution (continuing without CLV capture): {e}")
        ...
        if match_id in fixture_map:
            ...
            if status == "Finished":
                ...
                if actual_winner:
                    ...
                    closing_odds = _extract_closing_odds(
                        odds_map.get(match_id),
                        f,
                        sig.get("bet_on", ""),
                    )
                    if not dry_run:
                        ...
                        print(
                            f"[DRY-RUN] SIGNAL_ID {sig['id']}: Match {match_id} Finished -> "
                            f"{result.upper()} (Winner: {actual_winner}, Closing Odds: {closing_odds})"
                        )
                ...
                if not dry_run:
                    update_signal_result(sig["id"], "push", None, closing_odds=closing_odds)
                else:
    ...
    for sig in pending:
        ...
        try:
            ...
            if age_hours > 72:
                ...
                print(f"    Run: python -m ingestion.resolve_matches --force {sig['id']} --outcome VOID")
        except Exception:
            pass
